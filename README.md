...
Вы могли заметить, что пароли к БД и SMTP-серверу лежат прямо в репозитории в `application.properties`. Я знаю, что так делать нельзя 🙂

Это осознанное решение для pet-проекта, чтобы любой человек мог склонировать репо и запустить его одной командой без танцев с бубном вокруг `.env`-файлов.

**Как это должно работать в production:**
- ❌ Никаких секретов в репозитории
- ✅ Переменные окружения (`.env` в `.gitignore`)
- ✅ GitHub Secrets для CI/CD пайплайнов

Здесь же всё оставлено открытым исключительно ради удобства демонстрации.
